...
const patchLiquid = text => text;

const docDirPath = path.join(__dirname, '..', 'content', 'docs');
...
problems.forEach((problem, problemIndex) => {
  const indexOneBased = problemIndex + 1;
  const filename = `${indexOneBased.toString().padStart(6, '0')}-${problem.titleSlug}.md`;
  fs.writeFile(
    path.join(docDirPath, filename),
    patchLiquid(toDoc(problem, indexOneBased)),
    (err) => {
      if (err) {
